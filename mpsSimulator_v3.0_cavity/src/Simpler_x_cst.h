...
namespace SIM {
	...
	class Simpler_x_cst : public Simulator <R,D,Simpler_x_cst<R,D,P>> {
...
#pragma omp parallel for
	...
	private:
		...
		__forceinline void makeRhs_v_q1r1() {
#if OMP
#pragma omp parallel for
#endif
			for (int p = 0; p < int(part->np); p++) {
				...
				const auto gd = part->grad(part->phi, p);
				const auto rhs = (1. / para.dt)* (part->vel1[p]) - (1. / para.rho)* gd + para.g;
				for (auto d = 0; d < D; d++) {
